...
var jsmpeg = window.jsmpeg = function( url, opts ) {
	...
	if( url instanceof WebSocket ) {
		...
		this.initSocketClient.bind(this)();
	} 
	else {
		this.load(url);
	}
...
jsmpeg.prototype.initBuffers = function() {	
	...
	this.canvas.width = $(".camera-canvas").width();
	this.canvas.height = $(".camera-canvas").height();
	
	if( this.gl ) {
		this.gl.useProgram(this.program);
		this.gl.viewport(0, 0, $(".camera-canvas").width(), $(".camera-canvas").height());
	}
	else {
		this.currentRGBA = this.canvasContext.getImageData(0, 0, $(".camera-canvas").width(), $(".camera-canvas").height());
